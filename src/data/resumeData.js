

export const educationData = [
    {
        title: "BA Visual Communication",
        year: "2022",
        description: "University of Tehran, Tehran, Iran",
    },
    {
        title: "High School Diploma in Mathematics and Physics",
        year: "2022",
        description: "FARZANEGAN 1 (NODET), Tehran, Iran",
    },
];

export const languageData = [
    { language: 'Farsi', proficiency: 'Native', additionalInfo: '' ,  link: ''},
    { language: 'English', proficiency: 'Fluent', additionalInfo: 'Ielts 7.5' ,  link: ''},
    { language: 'Italian', proficiency: 'Proficient', additionalInfo: 'Cils b2' , link: 'https://online.unistrasi.it/qr_cils.asp?ID=29620201314306' },
    { language: 'German', proficiency: 'Proficient', additionalInfo: 'TestDaf c1', link: 'https://online.unistrasi.it/qr_cils.asp?ID=29620201314306' },
];

export const workExperienceData = [
    {
        title: "Freelance Photographer and Designer",
        year: "2014 - Present",
        description: "Self-Employed",
        list: [
            "Execute personal creative projects, exploring diverse artistic expressions.",
            "Collaborate with clients to deliver commissioned photography and design solutions.",
            "Showcase a versatile portfolio encompassing photography, graphic design, and visual communication.",
        ],
    },
    {
        title: "Graphic Designer",
        year: "2022 - Present",
        description: "Parla Idea, Tehran, Iran",
        list: [
            "Lead material design for consistent branding and visual appeal.",
            "Drive packaging design for eye-catching and functional solutions.",
            "Craft website design strategies for enhanced user experience and business alignment.",
            "Maintain the company's visual identity, ensuring brand cohesion.",
        ],
    },
    {
        title: "Teacher of 'Art History' and 'Visual Creativity'",
        year: "2019 - 2021",
        description: "Kooke Talaei Art School, Tehran, Iran",
        list: [
            "Fostered creativity through hands-on instruction.",
            "Mentored students in developing their unique visual style.",
        ],
    },
    {
        title: "Project-Based Translator",
        year: "Since 2019",
        list: [
            "Translated diverse titles with precision.",
            "Maintained nuance in academic and literary translations.",
        ],
    },
];
export const honorsData = [
    {
        title: "Certificate of Css, Html, GitHub and Web Design",
        year: "2023",
        description: "Codecademy",
    },
    {
        title: "Winner of Jan Michalski Foundation's Literary Grant",
        year: "2020",
        description: "La Fondation Jan Michalski, Switzerland",
    },
    {
        title: "Ranked 1st in the Nationwide University Entrance Exam (Konkur) among 48,000 Candidates",
        year: "2017",
        description: "National Organization of Educational Testing, Iran",
    },
    {
        title: "Member of the National Elite Foundation",
        year: "Since 2017",
        description: "National Elite Foundation, Iran",
    },
    {
        title: "Admission to the School of Exceptional Talents (Farzanegan)",
        year: "2010",
        description: "National Organization for Development of Exceptional Talents, Iran",
    },


    {
        title: "Certificate of Participation in Iran Open RoboCup for 3D Simulation",
        year: "2016",
        description: "Codecademy",
    },
    {
        title: "Certificate of Adobe Photoshop and Adobe Illustrator",
        year: "2014",
        description: "Tehran Institute of Technology",
    },

];

export const competenceData = [
    {
        title: 'Graphics and Imaging',
        items: [
            { name: 'Photoshop', percentage: 90 },
            { name: 'Lightroom', percentage: 80 },
            { name: 'Illustrator', percentage: 85 },
            { name: 'InDesign', percentage: 55 },
        ],
    },
    {
        title: 'Design and Prototyping',
        items: [
            { name: 'Figma', percentage: 85 },
            { name: 'Adobe XD', percentage: 75 },
        ],
    },
    {
        title: '3D Design',
        items: [
            { name: 'Blender', percentage: 75 },
            { name: 'Three.js', percentage: 30 },
        ],
    },

    {
        title: 'Web Development',
        items: [
            { name: 'HTML / CSS', percentage: 90 },
            { name: 'JavaScript', percentage: 85 },
            { name: 'React / Next.js', percentage: 80 },
            { name: 'Swift / Swift UI', percentage: 30 },
            { name: 'Git / Github', percentage: 75 },
        ],
    },
];